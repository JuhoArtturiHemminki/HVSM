...
impl HsvmEngine {
    ...
    pub fn predict_probability(&self) -> u32 {
        ...
        let p1_s = self.stats_short[idx_s][1] as f64 / (self.stats_short[idx_s][0] + self.stats_short[idx_s][1]) as f64;
        let p1_g = self.stats_ghost[idx_g][1] as f64 / (self.stats_ghost[idx_g][0] + self.stats_ghost[idx_g][1]) as f64;
    ...
    pub fn update_and_learn(&mut self, bit: u8) {
        ...
        let p_s = self.stats_short[idx_s][1] as f64 / (self.stats_short[idx_s][0] + self.stats_short[idx_s][1]) as f64;
        let p_g = self.stats_ghost[idx_g][1] as f64 / (self.stats_ghost[idx_g][0] + self.stats_ghost[idx_g][1]) as f64;
...
pub fn hsvm_decompress_file(source_path: &str, target_path: &str) -> Result<()> {
    ...
    for _ in 0..target_length {
        ...
        for i in (0..8).rev() {
            let p1 = engine.predict_probability();
            let r_one = (range >> 12) * p1;
            let boundary = low.wrapping_add(range - r_one);
            
            let bit = if code >= boundary { 1 } else { 0 };
